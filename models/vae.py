...
class FeedForward(nn.Module):
    ...
    def __init__(self, dims: List[int], activation_fn=F.relu, 
...
class Encoder(nn.Module):
    def __init__(self, x_dim: int, h_dims: List[int], z_dim: int, 
                 num_categories: int, tau2: float):
        ...
        self.sample_y = GumbelSoftmaxSample(in_features=h_dims[-1], 
                                            num_categories=num_categories, tau=tau2)
...
class Decoder(nn.Module):
    ...
    def __init__(self, z_dim: int, h_dims: List[int], x_dim: int):
        super().__init__()
        self.layers = FeedForward([z_dim] + h_dims + [x_dim], 
                                        output_activation_fn=F.sigmoid)
...
class SSLVAE(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = Encoder(self.x_dim, self.h_dims, self.z_dim, 
                               self.num_categories, self.tau2)
    ...
    def forward(self, x: torch.Tensor):
        ...
        kl_z = torch.sum(-0.5 * log_var + 0.5 * mu.pow(2) + 0.5 * log_var.exp() - 0.5).mean()
        ...
        qy = RelaxedOneHotCategorical(temperature=torch.tensor([self.tau2]), 
                                      logits=log_logits_y).log_prob(reparam_y)
        # uniform concrete prior
        # log logits = [-logK, -logK, ... , -logK]
        prior_log_logits = -torch.log(torch.tensor(self.num_categories, device=self.device))
        prior_log_logits = prior_log_logits.expand_as(log_logits_y)
        py = RelaxedOneHotCategorical(temperature=torch.tensor([self.tau1]), 
                                      logits=prior_log_logits).log_prob(reparam_y)
        kl_y = (qy - py).mean()
