...
def retain_mnist_classes(dataset: datasets.Dataset, 
                         allowed_labels: List[int]) -> datasets.Dataset:
...
def split_labeled_unlabeled(dataset: datasets.Dataset, 
                            label_ratio: float = 0.1, validation_ratio: float = 0.3) -> DatasetDict:
    """
    ...
    return DatasetDict({"labeled": labeled_dataset, "unlabeled": unlabeled_dataset, "validation": validation_dataset})


def load_mnist_for_ssl(allowed_labels: List[int] = [1, 2, 3, 4], 
                       label_ratio: float = 0.1, validation_ratio: float = 0.3) -> DatasetDict:
    """
...
if __name__ == "__main__":
    ...
    # put in args
    mnist_ssl = load_mnist_for_ssl(allowed_labels=[1, 2, 3, 4], label_ratio=0.1, validation_ratio=0.3)
